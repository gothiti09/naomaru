...
<button type="{{ $type }}" {{ $disabled ? 'disabled' : '' }} {{ $attributes }}
    class="inline-flex items-center justify-center border border-transparent font-medium rounded text-{{ $color }}-700 bg-{{ $color }}-100 hover:bg-{{ $color }}-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-{{ $color }}-500 {{ $class }}">
    {{ $slot }}
</button>
