        ...
        <div class="bg-white shadow overflow-hidden rounded-lg mb-4">
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl class="sm:divide-y sm:divide-gray-200">
                    ...
                    <x-molecules.dd-dt label="提案内容" required=true>
                    ...
                    <x-molecules.dd-dt label="加工のステージ">
                        @foreach ($stages as $stage)
                            <x-atoms.checkbox name="stages[]" id="stages[{{ $stage->id }}]" value="{{ $stage->id }}" label="{{ $stage->title }}"
                                checked="{{ $project->stages?->contains('project_id', $stage->id) }}" />
                        @endforeach
                    </x-molecules.dd-dt>
                    <x-molecules.dd-dt label="想定の加工方法">
                        @foreach ($methods as $method)
                            <x-atoms.checkbox name="methods[]" id="methods[{{ $method->id }}]" value="{{ $method->id }}" label="{{ $method->title }}"
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl class="sm:divide-y sm:divide-gray-200">
                    <x-molecules.dd-dt label="予算"
                        required=true>
                        <x-atoms.checkbox @click="openBudget=!openBudget" id="budget_undecided" name="budget_undecided" label="予算未定"
